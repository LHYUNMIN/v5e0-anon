"""MMMU clean re-measure + VQAv2 acc fix on uncontended GPU 0."""
...
print('=== MMMU + VQAv2 fix on Qwen2-VL-2B (GPU 0) ===', flush=True)
...
def vqav2_acc(pred, gt_answers):
    """VQAv2 official accuracy: min(matches/3, 1). gt_answers is list of 10 dicts."""
    if not gt_answers: return None
    pred_l = pred.lower().strip().rstrip('.,!?')
    matches = sum(1 for ans in gt_answers if str(ans.get('answer','')).lower().strip() == pred_l)
...
ds = load_dataset('lmms-lab/MMMU', split='validation', streaming=True)
samples = []
for s in ds:
...
print(f'  {len(samples)} MMMU samples', flush=True)
...
for i, s in enumerate(samples):
    try:
        ...
        if (i+1) % 50 == 0:
            ...
            print(f'  MMMU {i+1}/{len(samples)}: AR {ar_a.mean():.2f}, V5e-0 {ssd_a.mean():.2f}, sp {(ssd_a/ar_a).mean():.3f}', flush=True)
    except Exception as e:
        print(f'  MMMU prompt {i} fail: {str(e)[:60]}', flush=True)
...
results['mmmu'] = {'n': len(ar_a),
       ...
       'text_match': float(text_match), 'letter_match': float(letter_match)}
print(f'\n=== MMMU (n={len(ar_a)}) ===')
print(f'  AR: {ar_a.mean():.2f} t/s, V5e-0: {ssd_a.mean():.2f} t/s, sp: {sp.mean():.3f} ± {sp.std():.3f}')
if ar_acc:
    print(f'  AR acc: {100*np.mean(ar_acc):.1f}%, V5e-0 acc: {100*np.mean(ssd_acc):.1f}%, letter match: {100*letter_match:.1f}%')

# --- VQAv2 acc fix ---
print('\n[loading VQAv2 n=200 with proper acc]', flush=True)
...
for s in ds:
    ...
    img = s.get('image')
    q = s.get('question')
    mca = s.get('multiple_choice_answer')
    answers = s.get('answers')
    if img is None or not q: continue
    samples.append({'image': img, 'question': q, 'mca': mca, 'answers': answers})
print(f'  {len(samples)} VQAv2 samples', flush=True)
...
for i, s in enumerate(samples):
    try:
        ...
        ssd_tps_list.append(ssd_tps)
        ssd_texts.append(proc.tokenizer.decode(ssd_gen, skip_special_tokens=True).strip())
        gts.append(s['answers'])
        if (i+1) % 50 == 0:
            ar_a = np.array(ar_tps_list); ssd_a = np.array(ssd_tps_list)
            print(f'  VQAv2 {i+1}/{len(samples)}: AR {ar_a.mean():.2f}, V5e-0 {ssd_a.mean():.2f}, sp {(ssd_a/ar_a).mean():.3f}', flush=True)
    except Exception as e:
        print(f'  VQAv2 prompt {i} fail: {str(e)[:60]}', flush=True)

ar_vqa = [vqav2_acc(p, g) for p, g in zip(ar_texts, gts)]
ssd_vqa = [vqav2_acc(p, g) for p, g in zip(ssd_texts, gts)]
ar_vqa = [v for v in ar_vqa if v is not None]
ssd_vqa = [v for v in ssd_vqa if v is not None]
text_match = sum(1 for a,s in zip(ar_texts, ssd_texts) if a==s) / max(len(ar_texts), 1)
ar_a = np.array(ar_tps_list); ssd_a = np.array(ssd_tps_list); sp = ssd_a/ar_a
results['vqav2_fixed'] = {'n': len(ar_a),
       'ar_tps': float(ar_a.mean()), 'ssd_tps': float(ssd_a.mean()),
       'sp_mean': float(sp.mean()), 'sp_std': float(sp.std()),
       'ar_acc': float(np.mean(ar_vqa)) if ar_vqa else None,
       'ssd_acc': float(np.mean(ssd_vqa)) if ssd_vqa else None,
       'text_match': float(text_match)}
print(f'\n=== VQAv2 (n={len(ar_a)}) ===')
print(f'  AR: {ar_a.mean():.2f} t/s, V5e-0: {ssd_a.mean():.2f} t/s, sp: {sp.mean():.3f} ± {sp.std():.3f}')
if ar_vqa:
    print(f'  AR acc: {100*np.mean(ar_vqa):.1f}%, V5e-0 acc: {100*np.mean(ssd_vqa):.1f}%, text match: {100*text_match:.1f}%')

with open('./results/mmmu_vqav2_fix.json', 'w') as f:
    json.dump(results, f, indent=2)
print('\nSaved ./results/mmmu_vqav2_fix.json')
